            ...
            <div class="flex justify-between mb-2">
                <div class="flex">
                    <template v-if="!image_url">
                        <button class="bg-gray-300 max-w-max p-2.5 rounded-full" @click.prevent="$router.push(`/users/${$route.params.id}`)">
                            <img src="~/assets/default-profile-icon.svg" alt="" class="h-5">
                        </button>
                    </template>
                    <template v-else>
                        <button @click.prevent="$router.push(`/users/${$route.params.id}`)">
